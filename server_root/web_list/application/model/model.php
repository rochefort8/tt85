...
class Model extends Connection {
	...
	function updateData($table, $data, $id) {
		...
		if (is_array($data) && count(data) > 0 && $id > 0) {
			$data['editor'] = $_SESSION['userid'];
			$data['updated'] = date('Y-m-d H:i:s');
			foreach ($data as $key => $value) {
				$array[] = $this->quote($key)." = '".$this->quote($value)."'";
			}
			$query = "UPDATE ".$table." SET ".implode(",", $array)." WHERE id = ".intval($id);
			$this->response = $this->query($query);
		}
		return $this->response;
	...
	function validator($field, $caption, $rule) {
		...
		if (!isset($this->post[$field])) {
			if (!is_array($rule) && strlen($rule) > 0) {
				$rule = array($rule);
			}
			if (is_array($rule) && count($rule) > 0) {
				foreach ($rule as $value) {
					if ($value == 'distinct') {
						...
						$error = $this->validrule($_POST[$field], $caption, $value);
	...
	function validrule($string, $caption, $rule) {
		
		if (strlen($rule) > 0) {
			$array = array($string, $caption);
			if (stristr($rule, ':')) {
			...
			if (method_exists($this->validation, $method)) {
				return call_user_func_array(array($this->validation, $method), $array);
			} else {
	...
	function isvalid($string, $rule) {
		...
		if (is_array($rule) && count($rule) > 0) {
			foreach ($rule as $value) {
				$error = $this->validrule($string, 'string', $value);
				if ($error && strlen($error) > 0) {
					$response = false;
				}
			}
	...
	function exportcsv($data, $field, $filename, $array = null) {
		...
		if (is_array($data) && count($data) > 0) {
			...
			$filing = new Filing;
			$filing->exportcsv($array, $filename);
		} else {
		...
		if (is_array($filelist) && count($filelist) > 0) {
			foreach ($filelist as $value) {
				if (file_exists($temporary.$value) && $value != 'empty') {
					$filename = mb_convert_encoding($value, 'UTF-8', 'SJIS, EUC-JP, UTF-8');
	...
	function uploaddecode($string) {
		...
		if (stristr(PHP_OS, 'win')) {
			$string = mb_convert_encoding($string, 'UTF-8', 'SJIS, EUC-JP, UTF-8');
		}
		return $string;
